...
- **Axler — *Linear Algebra Done Right***: Vector spaces, linear maps, dual spaces. Read this to make the primal-dual structure second nature.
- **Boyd & Vandenberghe — *Convex Optimization***: Strong duality theorem, KKT conditions, LP duality. Free PDF at [stanford.edu/~boyd/cvxbook](https://stanford.edu/~boyd/cvxbook).

### Core: Discrete-Time Mathematical Finance

- **Capiński & Zastawniak — *Mathematics for Finance***: The book these exercises come from. One-period models, multi-period binomial trees, continuous-time introduction. Best starting point.
- **Pliska — *Introduction to Mathematical Finance: Discrete Time Models***: More rigorous treatment of state prices, FTAP proof, duality. Excellent complement to Capiński.

### Bridge: Discrete to Continuous

- **Björk — *Arbitrage Theory in Continuous Time***: The natural next step. First chapter reviews discrete models, then extends everything (replication, EMM, Girsanov) to continuous time. Black-Scholes derived rigorously.

### Advanced: Full Theory

- **Cochrane — *Asset Pricing***: Unifies all pricing methods under the SDF framework ($p = E[mX]$). Shows how CAPM, APT, and options pricing are special cases of the same structure. Free PDF at [John Cochrane's website](https://www.johnhcochrane.com/asset-pricing).
- **Duffie — *Dynamic Asset Pricing Theory***: Full measure-theoretic treatment. Arrow-Debreu equilibrium, martingale pricing, continuous-time completeness. Mathematically demanding but definitive.

### Learning Path

```
Axler + Boyd        →    Capiński (current)    →    Björk    →    Cochrane / Duffie
(Linear algebra,         (Discrete models,          (Continuous      (Full theory,
 LP duality)              FTAP, AD prices)            time, BS)        SDF framework)
```
...
All six methods are implemented in the accompanying Python files:

- `problem13.py` — Daily returns, covariance matrices (Dow Jones, Nasdaq, S&P 500)
- `problem15.py` — One-period market model, payoff matrix, replication, risk-neutral pricing
- `method6_utility.py` — CARA utility indifference pricing, sensitivity analysis
- `matrix_problems.py` — Covariance ↔ correlation matrix conversion (Problems 11 & 12)
- `returns_analysis.py` — Multi-index returns analysis, daily/weekly/monthly frequencies

```python
import numpy as np

# Core: all pricing from the payoff matrix D
S0, Su, Sd, r, T, K = 100, 120, 90, 0.05, 1.0, 105
BT = np.exp(r * T)
D  = np.array([[BT, Su], [BT, Sd]])
V  = np.array([max(Su-K,0), max(Sd-K,0)])   # call payoffs
p0 = np.array([1.0, S0])                     # prices today

# Method 1: Replication
phi = np.linalg.solve(D, V)
C1  = phi @ p0                               # 7.196

# Method 3: Arrow-Debreu
psi = np.linalg.solve(D.T, p0)
C3  = psi @ V                                # 7.196

# Method 2: Risk-neutral
q   = (S0*BT - Sd) / (Su - Sd)
C2  = np.exp(-r*T) * (q*V[0] + (1-q)*V[1]) # 7.196

# Method 4: SDF (assuming true prob p_u=0.6)
p_u, p_d = 0.6, 0.4
m   = psi / np.array([p_u, p_d])
C4  = p_u*m[0]*V[0] + p_d*m[1]*V[1]        # 7.196

assert np.allclose([C1, C2, C3, C4], 7.196, atol=1e-3)
```

---

*Notes compiled from exercises in Capiński & Zastawniak, Chapter 1, with extensions to all six pricing frameworks and connections to continuous-time theory.*
